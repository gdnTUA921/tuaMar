...
function DragNdrop({ onImagesChange, initialImages = [] }) {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop: (files) => {
      if (window.__startUpload) window.__startUpload();       // PREVENT auto logout
      handleChange(files);
      setTimeout(() => {
        if (window.__stopUpload) window.__stopUpload();
      }, 1000); // resume after drop stabilizes
    },
    accept: {
      "image/jpeg": [".jpeg", ".jpg"],
      "image/png": [".png"],
  ...
  return (
    <div className="drag-n-drop-container">
      ...
      <div className="dropzone-wrapper" {...getRootProps()}>
        ...
        <div
          className={`custom-dropzone ${isDragActive ? "drag-active" : "drag-inactive"
            }`}
        >
